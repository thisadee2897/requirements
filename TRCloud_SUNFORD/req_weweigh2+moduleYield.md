# ขอบเขตงาน WeWeigh2 + แอปใหม่โมดูลรับเข้า/Yield

**ชื่อโครงการ:** ใช้งาน WeWeigh2 สำหรับงานชั่งน้ำหนัก เพิ่มปุ่ม Export รายงาน และพัฒนาแอปใหม่เฉพาะโมดูลรับเข้า/Yield เชื่อมต่อ TRCloud
...
ส่วนการส่งข้อมูลสินค้าไปยัง TRCloud และงานรับเข้าเพื่อบันทึก Yield ให้ดำเนินการใน **แอปใหม่แยกจาก WeWeigh2** โดยแอปใหม่นี้มีเพียงโมดูลรับเข้า/Yield โมดูลเดียว และไม่มีการเพิ่มหรือเปลี่ยนแปลง Flow รับเข้าภายใน WeWeigh2

ขอบเขตโครงการประกอบด้วย:

1. ใช้งานโมดูล Weighing เดิมของ WeWeigh2
2. เพิ่มปุ่ม Export ในหน้ารายงานของ WeWeigh2
3. พัฒนาแอปใหม่ที่มีเฉพาะโมดูลรับเข้า/Yield
4. เพิ่มฟังก์ชันสนับสนุนสำหรับส่งข้อมูลสินค้าไปยัง TRCloud ภายในโมดูลรับเข้า/Yield
5. ค้นหารายการรับเข้า บันทึกน้ำหนักของเสีย และคำนวณ Yield ในแอปใหม่
...
### 2.2 แอปใหม่: โมดูลรับเข้า/Yield

แอปใหม่มีโมดูลธุรกิจเพียงโมดูลเดียวคือ **รับเข้า/Yield** โดยรับผิดชอบงานต่อไปนี้:
...
Login, การกำหนดสิทธิ์, Product Upload, Barcode/SN Lookup, การเชื่อมต่อ API และ Transaction Log เป็นฟังก์ชันสนับสนุนภายในโมดูลรับเข้า/Yield ไม่ถือเป็นโมดูลธุรกิจแยกต่างหาก
...
## 5. ฟังก์ชันส่งข้อมูลสินค้าไปยัง TRCloud ภายในโมดูลรับเข้า/Yield
...
## 6. แอปใหม่: การรับเข้าและค้นหาด้วย Barcode/SN

- หน้าจอรับเข้า/Yield ต้องอยู่ในแอปใหม่ ไม่ใช่ WeWeigh2
...
## 7. แอปใหม่: การบันทึกของเสียและ Yield

### 7.1 ขั้นตอนการทำงาน

```mermaid
sequenceDiagram
    participant N as แอปใหม่ Inbound/Yield
...
- รับน้ำหนักของเสียตามวิธีที่ยืนยันสำหรับแอปใหม่
...
- ส่งมอบ APK และ Android App Bundle ของแอปใหม่โมดูลรับเข้า/Yield
...
- โมดูลอื่นภายในแอปใหม่นอกเหนือจากโมดูลรับเข้า/Yield
- Dashboard, Formulation, การชั่งตามสูตร, เบิก, โอนย้าย, คืนสินค้า หรือบริหารคลังภายในแอปใหม่
...
- ชื่อแอป App Icon และรูปแบบหน้าจอของแอปใหม่
...
6. แอปใหม่มีเพียงโมดูลรับเข้า/Yield และฟังก์ชันสนับสนุนที่ระบุ
7. แอปใหม่ส่งข้อมูลสินค้าไป TRCloud ได้โดยไม่สร้างข้อมูลซ้ำ
8. แอปใหม่จัดเก็บ TRCloud Item ID หรือเลขอ้างอิงได้
9. แอปใหม่ค้นหารายการรับเข้าด้วย Barcode/SN ได้
10. แอปใหม่บันทึกน้ำหนักของเสียและคำนวณ Yield ตามสูตรที่ยืนยันได้
11. แอปใหม่แสดง Transaction ID และสถานะรายการจาก TRCloud ได้
12. เมื่อ API ขัดข้อง รายการสามารถ Retry ได้โดยไม่สร้างข้อมูลซ้ำ
