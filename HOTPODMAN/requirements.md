# Hotpot Man — Business Requirements

## 1. บทสรุปโครงการ

โครงการนี้มีเป้าหมายเพื่อพัฒนาระบบติดตาม PO จาก ERP และบริหารคลังสินค้าสำหรับธุรกิจร้านอาหาร Hotpot Man ให้สามารถติดตามสินค้าและวัตถุดิบตั้งแต่ PO รับเข้าคลัง จัดเก็บ ย้าย ตรวจนับ เบิก แปรรูป คืน ตลอดจนส่งข้อมูลให้ระบบบัญชีหรือ ERP

ERP เป็นระบบหลักสำหรับการสร้างและจัดการเอกสาร PO และข้อมูล Master ทั้งหมดที่ ERP มี Hotpot Man รับข้อมูล PO ผ่าน Webhook และดึง/Sync Master โดยใช้ ID จาก ERP เป็น Primary Key หากข้อมูล Master ตารางที่เกี่ยวข้อง หรือความสัมพันธ์ที่จำเป็นต่อฟังก์ชันในขอบเขตนี้ไม่มีใน ERP ทีม SUNFORD จะพัฒนาเพิ่มเติมในระบบ Hotpot Man เพื่อให้ใช้งานได้ครบ โดยลดการตั้งค่าข้อมูลที่มีอยู่แล้วซ้ำหลายระบบ
...
- การสร้าง แก้ไข อนุมัติ ยกเลิก หรือปิดเอกสาร PO ใน Hotpot Man; การจัดการเอกสาร PO ทำใน ERP
- การสร้างหรือแก้ไข Master ที่ ERP ดูแลซ้ำใน Hotpot Man; ข้อนี้ไม่รวม Master หรือข้อมูลเสริมที่ ERP ไม่มีและ SUNFORD พัฒนาเพิ่มตาม BR-01
...
```mermaid
flowchart TD
    ERP[ERP: จัดการเอกสาร PO] -->|Webhook| A[Hotpot Man: รับและเก็บข้อมูล PO]
    A --> B[Management Web: ค้นหาและตรวจสอบ PO]
    B --> C[Management Web: รับสินค้า]
    PRODUCT[ERP: Master และข้อมูลที่เกี่ยวข้อง] -->|Sync โดยใช้ ID จาก ERP| MASTER[Hotpot Man: Master และความสัมพันธ์]
...
- สร้าง แก้ไข อนุมัติ หรือยกเลิก PO ผ่าน ERP แล้วติดตามข้อมูลที่ส่งมายัง Hotpot Man
...
- มีฟีเจอร์ Sync ข้อมูลดังกล่าวเข้าฐานข้อมูล Hotpot Man ทั้งการนำเข้าครั้งแรกและการอัปเดตภายหลัง เพื่อใช้กับ PO รับสินค้า คลัง และรายงานโดยไม่ต้องตั้งค่า Master ที่ ERP มีซ้ำ
...
- หาก ERP ไม่มี Master รายละเอียด หรือตารางที่จำเป็นต่อฟังก์ชันในขอบเขตนี้ ทีม SUNFORD พัฒนาเพิ่มเติมใน Hotpot Man พร้อมหน้าจอจัดการตามสิทธิ์
...
- มีตารางที่เกี่ยวข้องและตารางเชื่อมความสัมพันธ์ตามจำเป็น พร้อม Foreign Key อ้างอิง Master ที่เก็บใน Hotpot Man เช่น สินค้า–หน่วย สินค้า–Supplier และสาขา–คลัง–Location รวมถึงการอ้างอิงจากรายการรับ เบิก แปรรูป และคืน
- ผู้มีสิทธิ์สั่ง Sync ซ้ำและดูสถานะ ผลการ Sync ล่าสุด หรือสาเหตุที่ไม่สำเร็จได้
- Master ที่ ERP ดูแลใช้สำหรับดูและอ้างอิง การแก้ข้อมูลต้นทางทำใน ERP แล้ว Sync เข้ามา ส่วนข้อมูลที่ ERP ไม่มีจัดการใน Hotpot Man ตามสิทธิ์
...
- รับข้อมูล PO และรายการสินค้าจาก ERP ผ่าน Webhook แล้วเก็บข้อมูลใน Hotpot Man เพื่อค้นหา เปิดดู และใช้อ้างอิงการรับสินค้า
- ERP เป็นผู้สร้าง แก้ไข อนุมัติ ยกเลิก และปิดเอกสาร PO โดย Hotpot Man ไม่มีหน้าจอหรือคำสั่งสำหรับจัดการเอกสาร PO เหล่านี้
- รับการเปลี่ยนแปลงข้อมูลและสถานะ PO จาก ERP โดยคงเลขอ้างอิงเอกสารต้นทาง
- แสดงจำนวนสั่ง จำนวนรับ และจำนวนค้างรับแต่ละรายการ
- แสดงสถานะเอกสารตาม ERP และแยกความคืบหน้าการรับสินค้าใน Hotpot Man เช่น ยังไม่รับ รับบางส่วน และรับครบ ออกจากสถานะเอกสารต้นทาง
...
- ข้อมูลที่เกี่ยวข้องในข้อนี้รวมรหัสอ้างอิงของ Master และความสัมพันธ์ที่ธุรกรรมต้องใช้ เช่น สินค้า Supplier หน่วย บริษัท สาขา คลัง และ Location โดยมีตาราง Master และตารางเชื่อมความสัมพันธ์ที่จำเป็นใน Hotpot Man พร้อม Foreign Key ตาม BR-01
- Foreign Key ใช้เชื่อมข้อมูลภายในฐานข้อมูล Hotpot Man ส่วนการส่งกลับ ERP ส่งข้อมูลธุรกรรมพร้อมรหัสอ้างอิงที่ Interface รองรับ
...
11. ระบบต้องใช้ DigitalOcean Cloud Infrastructure โดยมี Application Server, Managed PostgreSQL Database, Private Object Storage, VPC, Firewall, Backup และ Monitoring ตาม Technical Baseline
12. UAT และ Production ต้องแยก Environment, PostgreSQL Database, Object Storage Bucket และ Credential ออกจากกัน

## 9. เกณฑ์ยอมรับระดับโครงการ

1. สามารถรับ PO ผ่าน Webhook จาก ERP ค้นหาและอ้างอิงเอกสารเพื่อรับสินค้าแบบเต็มจำนวน/บางส่วนได้ โดยไม่มีการสร้างหรือจัดการเอกสาร PO ใน Hotpot Man
...
25. Master และข้อมูลเสริมที่ ERP ไม่มีแต่จำเป็นต่อฟังก์ชันในขอบเขตนี้สามารถจัดการใน Hotpot Man ตามสิทธิ์ โดยไม่ชนกับ ID ของ ERP และไม่ถูก Sync ทับ
...
```mermaid
gantt
    title Hotpot Man — Implementation Plan 60 Days
...
|---|---|
| ฐานข้อมูลและ ERP | ระบบ Hotpot Man ใช้ PostgreSQL Database แยกจาก ERP และเชื่อมต่อผ่าน API/Integration โดยไม่เขียนตรงลงตาราง ERP |
| Cloud Infrastructure | Hotpot Man เปิดบัญชี DigitalOcean ของบริษัทและเป็นเจ้าของค่าใช้จ่ายตั้งแต่เริ่มโครงการ; SUNFORD ใช้สิทธิ์ที่บริษัทมอบให้เพื่อติดตั้ง Droplet สำหรับ Web/API, Managed PostgreSQL, Private Spaces สำหรับไฟล์บิล/OCR/Claim/Export, VPC, Firewall, Backup และ Monitoring; ขนาดเริ่มต้นต้องประเมินจากปริมาณใช้งานจริงก่อน Production Rollout |
| Environment | Hotpot Man สร้างทั้ง UAT และ Production Project ภายใต้บัญชี DigitalOcean ของบริษัทตั้งแต่เริ่มโครงการ; UAT เปิดพร้อมเริ่มโครงการเพื่อทดสอบ ERP ส่วน Production แยกจาก UAT และต้องผ่าน Deploy, Domain/SSL, Firewall, Backup/Restore และ Monitoring ก่อนใช้งานจริง |
| PO | สร้างและจัดการเอกสารใน ERP; Hotpot Man รับข้อมูลผ่าน Webhook เพื่ออ้างอิงรับสินค้าและติดตามยอด |
...
- ระบบ Hotpot Man ใช้ PostgreSQL Database แยกจาก ERP และไม่เขียนตรงลงตาราง ERP
- ERP เป็นเจ้าของเอกสาร PO และ Master ที่มีอยู่; Hotpot Man รับ PO ผ่าน Webhook และ Sync Master โดยใช้ ID จาก ERP ส่วนข้อมูลที่ ERP ไม่มีและจำเป็นต่อขอบเขตงานให้ SUNFORD พัฒนาเพิ่มพร้อมความสัมพันธ์และ Foreign Key
